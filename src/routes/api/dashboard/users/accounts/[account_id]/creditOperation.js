...
const creditOperationSchema = Joi.object({
  ...
  payment_method: Joi.string().required().allow(""),
}).unknown(true);
...
export const POST = async (req, res) => {
  const { value, error } = creditOperationSchema.validate({
    ...req.query,
    ...req.body,
  });
  if (error) {
  ...
  if (!utilFunctions.checkDecimalsForCurrency(account.currency, value.amount)) {
    throw new HTTPError("Invalid amount", 400, errorCodes.invalidAmount);
  }
  if (value.type === "deposit") {
    const transaction =
      await userTransactionsService.depositManualCreditOperation(
        account.user._id,
        account._id,
        value.amount,
        account.currency,
    ...
    account = await userAccountsService
      .depositToAccountBonus(account._id, transaction, value.reason)
      .catch(async (err) => {
  ...
  if (value.type === "withdrawal") {
    const transaction =
      await userTransactionsService.withdrawManualCreditOperation(
        account.user._id,
        account._id,
        value.amount,
        account.currency,
    ...
    account = await userAccountsService
      .withdrawFromAccountBonus(account._id, transaction)
      .catch(async (err) => {
